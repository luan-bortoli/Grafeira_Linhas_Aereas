...
template <typename T> Filapri_min<T>::Filapri_min(int max_num_itens) {
  if (max_num_itens <= 0) {
...
template <typename T> bool Filapri_min<T>::vazia() { return (num_itens_ == 0); }

template <typename T> bool Filapri_min<T>::cheia() {
  return (num_itens_ == max_num_itens_);
}

template <typename T> bool Filapri_min<T>::tem_indice(int ind) {
  try {
    valida_indice(ind);
  } catch (...) {
...
template <typename T> void Filapri_min<T>::insere(int ind, T prio) {
  if (cheia()) {
  ...
  if (tem_indice(ind)) {
    throw(runtime_error("Erro na operacao insere: o indice " + to_string(ind) +
                        " do item ja existe na fila de prioridade!"));
  }
...
template <typename T> pair<int, T> Filapri_min<T>::remove() {
  if (vazia()) {
...
template <typename T> T Filapri_min<T>::prio(int ind) {
  try {
    valida_indice(ind);
  } catch (...) {
...
template <typename T> void Filapri_min<T>::diminui_prio(int ind, T prio) {
  try {
    valida_indice(ind);
  } catch (...) {
  ...
  if (prio >= itens_[ind_posfila_[ind]].second) {
    throw(runtime_error("Erro na operacao diminui_prio: a nova prioridade"
                        " (" +
                        to_string(prio) + ") do item de indice " +
                        to_string(ind) +
...
template <typename T> int Filapri_min<T>::pai(int i) { return ((i - 1) / 2); }

template <typename T> int Filapri_min<T>::filho_esq(int i) {
  return ((2 * i) + 1);
}

template <typename T> int Filapri_min<T>::filho_dir(int i) {
  return ((2 * i) + 2);
}

template <typename T> void Filapri_min<T>::troca(int i, int j) {
...
template <typename T> void Filapri_min<T>::conserta_para_cima(int i) {
  while (i > 0) {
...
template <typename T> void Filapri_min<T>::conserta_para_baixo(int i) {
  int menor_filho;
...
template <typename T> void Filapri_min<T>::valida_indice(int ind) {
